...
st.markdown("# Dashboard Estoque — Tidimar Hospitalar")
st.markdown(
...
# Extrai meses únicos (ordenados) e separadores/conferentes/dias únicos
...
# Extrai dias únicos (1-31) e ordena
dias_unicos = sorted(df['DATA'].dt.day.unique())
dias_opcoes = ["Todos"] + [str(d) for d in dias_unicos]
...
# Filtro de dia
if "Todos" not in dias_selecionados:
    dias_nums = [int(d) for d in dias_selecionados]
    df_filtrado = df_filtrado[df_filtrado['DATA'].dt.day.isin(dias_nums)]
